...
class PluginBanneroid_ModuleBanner extends Module
{
    ...
    public function Init()
    {
        ...
        $this->_aPlaceNames[0] = E::ModuleLang()->Get('plugin.banneroid.banneroid_total');
        $this->_aPlaceNames[1] = E::ModuleLang()->Get('plugin.banneroid.banneroid_under_article');
        $this->_aPlaceNames[2] = E::ModuleLang()->Get('plugin.banneroid.banneroid_side_bar');
        $this->_aPlaceNames[3] = E::ModuleLang()->Get('plugin.banneroid.banneroid_body_begin');
        $this->_aPlaceNames[4] = E::ModuleLang()->Get('plugin.banneroid.banneroid_body_end');
        ...
        if (is_array($aRows) && count($aRows)) {
            foreach ($aRows as $aRow) {
                ...
                foreach ($aPages as $aRow) {
                    $sPages .= " " . E::ModuleLang()->Get('plugin.banneroid.' . $aRow['place_name']) . '(' .
        ...
        if (isset($_REQUEST['submit_banner'])) { // Update or insert banner
            ...
            if (!preg_match(Config::Get('plugin.banneroid.banner_date_reg'), $sStartDate)) {
                E::ModuleMessage()->AddError(
                    E::ModuleLang()->Get("plugin.banneroid.banneroid_error_date_start"),
                    E::ModuleLang()->Get('plugin.banneroid.banneroid_error'));
                $bStateError = 1;
            }
            
            if (!preg_match(Config::Get('plugin.banneroid.banner_date_reg'), $sEndDate)) {
                E::ModuleMessage()->AddError(
                    E::ModuleLang()->Get("plugin.banneroid.banneroid_error_date_end"),
                    E::ModuleLang()->Get('plugin.banneroid.banneroid_error'));
                $bStateError = 1;
            }
            ...
            if (!func_check($_REQUEST['banner_name'], 'text', 2, 3000)) {
                E::ModuleMessage()->AddError(
                    E::ModuleLang()->Get("plugin.banneroid.banneroid_error_name"),
                    E::ModuleLang()->Get('plugin.banneroid.banneroid_error'));
                $bStateError = 1;
            }
            ...
            if (!preg_match(Config::Get('plugin.banneroid.banner_url_reg'), $_REQUEST['banner_url']) and !$_REQUEST['banner_html']) {
                E::ModuleMessage()->AddError(
                    E::ModuleLang()->Get("plugin.banneroid.banneroid_error_url"),
                    E::ModuleLang()->Get('plugin.banneroid.banneroid_error'));
                $bStateError = 1;
            }
            ...
            if (in_array('l10n', $this->aActivePlugins)) {
                ...
                if ($sLang === '0') {
                    ...
                    if (!in_array($sLang, $aLangs)) {
                        E::ModuleMessage()->AddError(
                            E::ModuleLang()->Get("plugin.banneroid.banneroid_error_lang"),
                            E::ModuleLang()->Get('plugin.banneroid.banneroid_error'));
                        $bStateError = true;
                    }
                }
            ...
            if (isset($_FILES["banner_image"]) && $_FILES["banner_image"]["error"] == 0) {
                ...
                if (!in_array($aSize['mime'], Config::Get('plugin.banneroid.images_mime'))) {
                    E::ModuleMessage()->AddError(
                        E::ModuleLang()->Get("plugin.banneroid.banneroid_error_image_extension"),
                        E::ModuleLang()->Get('plugin.banneroid.banneroid_error'));
                    $iOk = 0;
                } else
                    ...
                    if (!$this->UploadImage($aImageFile, $oBanner)) {
                        E::ModuleMessage()->AddError(
                            E::ModuleLang()->Get("plugin.banneroid.banneroid_error_unable_to_upload_image"),
                            E::ModuleLang()->Get('plugin.banneroid.banneroid_error'));
                        $iOk = 0;
                    }
            }
            ...
            if (!isset($iRes)) { //show error editiding banner
                E::ModuleMessage()->AddError(
                    E::ModuleLang()->Get("plugin.banneroid.banneroid_error_edit"),
                    E::ModuleLang()->Get('plugin.banneroid.banneroid_error'));
                return false;
